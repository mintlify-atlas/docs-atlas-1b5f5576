---
title: "How the EnglishMatrix Sentence Engine Works"
sidebarTitle: "Sentence Engine"
description: "Technical reference for GenerateSentence, BuildEnglishVerb, and BuildSpanishVerb — the three functions that produce bilingual English/Spanish sentence pairs in EnglishMatrix."
---
...
2. **Verb:** A random entry is picked from `verbsForSentences` (64 entries).
...
| `"She"`, `"He"`, `"My mom"`, `"The teacher"`, and all other 3rd-person singulars | él/ella (3rd singular) | `ElPres`, `ElPast` |
| `"We"`, `"They"`, `"You"` | nosotros/ellos/ustedes | `NosPres`, `NosPast` |

The internal booleans are set as:
...
Any subject that is neither `"i"` nor `"we"` (including `"they"` and `"you"`) uses the `El`/`El` (third-person) fields.
